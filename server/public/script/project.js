...
projectForm.addEventListener('submit', async (event) => {
    ...
    try{
        const result = await fetch("https://morning-atoll-88297.herokuapp.com/api/project", {
            method: "POST",
            headers: {
...
function addDeleteProjectListener(listElement, projectId, deleteButton){
    deleteButton.addEventListener('click',  async (event) => {
        try{
           const result = await fetch(`https://morning-atoll-88297.herokuapp.com/api/project/${projectId}`,
            {
                method: "DELETE",
                headers: {
                    'Authorization': `Bearer ${token}`
...
projectEditForm.addEventListener('submit', async (event) => {
    ...
    try{
        const result = await fetch(`https://morning-atoll-88297.herokuapp.com/api/project/${selectedProjectId}`, {
            method: "PUT",
            headers: {
                'Authorization': `Bearer ${token}`
            },
